...
from attack.Ascon.AsconXOF.final_result.Ascon_XOF_round_3_preimage_for_painting import *
import os

# Create Gurobi model
model = gp.Model("Ascon_MILP_Automation")
model.setParam('MIPFocus', 2)
# model.setParam('MIPGap', 0.07)  # Set optimality gap to 0
...
model.setObjectiveN(delta_total_r-1 * sum_linear_cancel , index=1, priority=1, name="SecondaryObjective")
model.setObjectiveN(-1 * sum_without_bits , index=2, priority=0, name="ThirdObjective")
...
for round_state in intermediate_states:
    ...
    without_place = round_state['without_place']
    temp = [[0 for x in range(5)] for z in range(slice_number)]
